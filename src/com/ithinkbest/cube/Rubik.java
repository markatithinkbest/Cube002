...
public class Rubik {
	...
	int ratateNameStyle=0;
	private String colorName=".紅藍黃橘綠白";
	...
	public final static String[][] ROTATE_NAME={
			//
			{"XL1","左上"},//
			{"XL2","左翻"},//
			{"XL3","左下"},//
...
//			System.out.println(cubes[i]);
	...
	public String getFaceColorNameTextImg(){
		StringBuilder sb=new StringBuilder();
		int temp;
		for (int k=0;k<6;k++){
			for (int i=0;i<3;i++){				
				temp=cubes[FACE_COLOR[k][i]].getFaceColor(k);
				sb.append(colorName.substring(temp,1+temp))
				.append(' ');
	...
	public void rotateShowNameAndResult(int k) {
		rotate(k);
		System.out.print(ROTATE_NAME[k][0]);
		System.out.println(ROTATE_NAME[k][1]);
		
		System.out.println(getFaceColorNameTextImg());
	}
